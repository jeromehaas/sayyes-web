...
const MobileNavigation = () => {
	...
	useEffect(() => {
		...
		return () => { return hamburgerRef.current.destroy(); };
	}, []);
	...
	return (
		<nav className="mobile-navigation" ref={ mobileNavigationRef }>
			<div className="mobile-navigation__bar bar">
				...
				<div className="bar__hamburger" ref={ hamburgerContainerRef } onClick={ toggleNavigation } />
			</div>
			<div className="mobile-navigation__menu menu">
				<Link className="menu__item" href="/" onClick={ (event) => { return navigate(event, '/'); } }>Home</Link>
				<Link className="menu__item" href="/buy" onClick={ (event) => { return navigate(event, '/buy'); } }>Kaufen</Link>
				<Link className="menu__item" href="/sell" onClick={ (event) => { return navigate(event, '/sell'); } }>Verkaufen</Link>
				<Link className="menu__item" href="/about" onClick={ (event) => { return navigate(event, '/about'); } }>About</Link>
				<Link className="menu__item" href="/collection" onClick={ (event) => { return navigate(event, '/collection'); } }>Kollektion</Link>
			</div>
